...
The idea is that you can mark a subset of points in the scatterplot by painting them by your mouse. Hitting buttons "clear current selection" removes all markings and "reverse selection" reverses the marking. "recompute projection" computes PCA projection vectors using the marked subset of points only and then projects the whole data to these first two principal components.
